...
describe('Testa a função multiplePricePerMinuteWithPlan()', () => {
  ...
  it('Verifica se ao passar parâmetro com o valor 0 a multiplicação da função multiplePricePerMinuteWithPlan() retorna o valor 0', () => {
...
describe('Testa a função sendQuote()', () => {
  it('Verifica se a multiplicação da função sendQuote() retorna o calcúlo correto', () => {
    expect(sendQuote(30, 1.90, 20)).toEqual({ withoutPlan: "38.00", withPlan: "0.00", })
    expect(sendQuote(60, 1.70, 80)).toEqual({ withoutPlan: "136.00", withPlan: "37.40", })
    expect(sendQuote(120, 1.90, 200)).toEqual({ withoutPlan: "380.00", withPlan: "167.20", })
    expect(sendQuote(30, 1.90, 100)).toEqual({ withoutPlan: "190.00", withPlan: "146.30", })
  })

  it('Verifica se ao passar parâmetro callTime com o valor 0 a multiplicação da função sendQuote() retorna o valor 0', () => {
    expect(sendQuote(60, 1.70, 0)).toEqual({ withoutPlan: "0.00", withPlan: "0.00" })
  })
});